class Admin::TableGroupsController < AdminController
  ...
  before_action :set_table_group, only: [:show, :edit, :update, :destroy, :reorder_tables, :toggle_active]
      ...
      respond_to do |format|
        format.turbo_stream do
          render turbo_stream: [
            ...
            turbo_stream.append('table-groups-tbody', partial: 'admin/table_groups/table_group_row', locals: { table_group: @table_group, global_priorities: {} })
          ]
        end
    ...
    if @table_group.update(table_group_params)
      respond_to do |format|
        format.turbo_stream do
          render turbo_stream: [
            turbo_stream.remove('modal'),
            turbo_stream.update('flash_messages', partial: 'shared/flash', locals: { message: '桌位群組更新成功', type: 'success' }),
            turbo_stream.replace("group_#{@table_group.id}", partial: 'admin/table_groups/table_group_row', locals: { table_group: @table_group, global_priorities: {} })
          ]
        end
