class Admin::TablesController < AdminController
      ...
      respond_to do |format|
        format.turbo_stream do
          render turbo_stream: [
            turbo_stream.remove('modal'),
            turbo_stream.update('flash_messages', partial: 'shared/flash',
                                                  locals: { message: '桌位更新成功', type: 'success' }),
      ...
      respond_to do |format|
        format.turbo_stream do
          render turbo_stream: turbo_stream.replace("table_#{@table.id}",
                                                    partial: 'table_card',
                                                    locals: { table: @table, restaurant: @restaurant,
                                                              priority: @table.sort_order || 1 })
        end
  ...
  def set_table
    @table = if @table_group
               @table_group.restaurant_tables.find(params[:id])
             else
               @restaurant.restaurant_tables.find(params[:id])
             end
  end
