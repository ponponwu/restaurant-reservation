class BusinessPeriod < ApplicationRecord
  ...
  # 新增支援 0-6 格式的 scope
  scope :for_weekday, lambda { |weekday|
  ...
  def days_of_week
    ...
    DAYS_OF_WEEK.select { |_day, bit| days_of_week_mask.anybits?(bit) }.keys.map(&:to_s)
  end
  ...
  def operates_on_day?(day)
    ...
    days_of_week_mask.anybits?(day_bit)
  end
  ...
  def operates_on_weekday?(weekday)
    ...
    days_of_week_mask.anybits?(bit)
  end

  def chinese_days_of_week
    days_of_week.filter_map { |day| CHINESE_DAYS[day.to_sym] }
  end
  ...
  def self.weekday_to_bit(weekday)
    ...
    # 0-6 格式轉換為 bitmask
    day_symbols = %i[sunday monday tuesday wednesday thursday friday saturday]
    day_symbol = day_symbols[weekday]
    DAYS_OF_WEEK[day_symbol]
  end

  def self.bit_to_weekday(bit)
    return nil unless bit&.positive?
