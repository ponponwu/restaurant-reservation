...
RSpec.describe '前端日曆整合測試', type: :system, js: true do
  ...
  before do
    ...
    # 給日曆更多時間載入
    expect(page).to have_css('.flatpickr-calendar', wait: 15)
  end
  ...
  describe '日曆與營業規則的整合' do
    ...
    context '當餐廳完全沒有桌位容量時' do
      ...
      it '應顯示無法訂位的訊息且不顯示日曆' do
        ...
        begin
          expect(page).to have_content(/無法.*安排訂位|無法.*預約|沒有可用的桌位/, wait: 10)
  ...
  describe '時間槽的即時更新' do
    ...
    it '當選擇的日期沒有任何可用時間時，應顯示提示訊息' do
      ...
      select_date_in_calendar(available_date)
      expect(page).to have_content(/該日無可用時間|沒有可用的時間|無可用時段|此日期無可用時間/, wait: 10)
    end
  end
  ...
  describe '處理 API 錯誤' do
    it '當獲取時間槽的 API 失敗時，應優雅地顯示錯誤訊息' do
      ...
      expect(page).to have_content(/載入.*失敗|錯誤|無法載入/, wait: 10)
    end
  end
