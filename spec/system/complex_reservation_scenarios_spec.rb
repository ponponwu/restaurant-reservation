...
RSpec.describe '複雜訂位情境系統測試' do
  ...
  describe '高峰時段訂位管理' do
    it '管理員應該能快速處理多個同時間訂位請求', :js do
      ...
      reservation_data.each_with_index do |data, _index|
        click_button '新增訂位'
        ...
        # 檢查是否成功建立
        successful_reservations += 1 if page.has_content?('訂位建立成功') || page.has_content?(data[:name])
  ...
  describe '併桌功能操作流程' do
    ...
    it '應該動態顯示併桌的可用組合', :js do
      ...
      within '#reservation_form' do
        ...
        # 觸發併桌檢查
        find_by_id('party_size').send_keys(:tab)
        ...
        within('.table-combination-options') do
          ...
          expect(page).to have_css('.combination-option', count: 1..3)
        end
      end
    end
  end
