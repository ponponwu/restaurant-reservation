module SystemTestHelpers
  # 安全執行系統測試的輔助方法
  def safely_run_system_test(&block)
  ...
  # 條件性等待
  def wait_for_condition(timeout: 10, &condition)
  ...
  def wait_for_ajax
    wait_for_condition(timeout: 30) do
      page.execute_script('return jQuery.active == 0') rescue true
    end
  end
...
RSpec.configure do |config|
  ...
  config.before(:each, type: :system) do
    unless browser_available?
      skip_system_test_with_message("Browser not available in test environment")
    end
  end
