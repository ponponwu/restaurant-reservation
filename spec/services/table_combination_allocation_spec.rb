  ...
  describe '基本拼桌場景' do
    ...
    context '4人桌被佔用後的拼桌需求' do
      it '唯一4人桌被佔用後，其他群組能拼出4人桌滿足新訂位' do
        ...
        raise '所有單一桌位都被佔用，應該使用拼桌解決方案' if new_reservation.table_combination.blank?
  ...
  describe '複雜併桌情境' do
    ...
    context '多層次併桌需求' do
      ...
      it '混合單桌和併桌的複雜分配情境' do
        # 先分配一些單桌訂位
        small_reservations = []
        [window_table_2, center_table_2a].each_with_index do |table, _index|
    ...
    context '併桌失敗和降級策略' do
      it '當併桌無法滿足需求時的處理' do
        ...
        occupied_tables.each_with_index do |table, _index|
          create(:reservation, :confirmed,
                 restaurant: restaurant,
                 business_period: business_period,
                 table: table,
