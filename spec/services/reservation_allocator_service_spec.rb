...
RSpec.describe ReservationAllocatorService, type: :service do
  ...
  describe '無限用餐時間模式' do
    ...
    context '基本無限時分配邏輯' do
      it '每餐期每桌只能有一個訂位' do
        # 第一個訂位佔用桌位
        first_reservation = create(:reservation, :confirmed,
                                 restaurant: restaurant,
                                 business_period: business_period,
                                 table: table1,
        ...
        second_reservation = build(:reservation,
                                 restaurant: restaurant,
                                 business_period: business_period,
                                 reservation_datetime: 1.day.from_now.change(hour: 13),
                                 party_size: 2)
        
        service = ReservationAllocatorService.new(second_reservation)
      ...
      it '不同餐期可以重複使用同一桌位' do
        ...
        # 午餐時段的訂位
        lunch_reservation = create(:reservation, :confirmed,
                                 restaurant: restaurant,
                                 business_period: business_period,
                                 table: table1,
                                 reservation_datetime: 1.day.from_now.change(hour: 12),
        ...
        # 晚餐時段的訂位，可以使用同一桌位
        dinner_reservation = build(:reservation,
                                 restaurant: restaurant,
                                 business_period: dinner_period,
                                 reservation_datetime: 1.day.from_now.change(hour: 18),
                                 party_size: 2)
        
        service = ReservationAllocatorService.new(dinner_reservation)
    ...
    context '容量檢查在無限時模式下' do
      it '按餐期計算容量而非時段' do
        restaurant.update!(total_capacity: 8)
    ...
    context '與限時模式的對比' do
      it '切換到限時模式後使用時間區間檢查' do
        # 先在無限時模式下創建訂位
        unlimited_reservation = create(:reservation, :confirmed,
                                     restaurant: restaurant,
                                     business_period: business_period,
                                     table: table1,
                                     reservation_datetime: 1.day.from_now.change(hour: 12),
        ...
        limited_reservation = build(:reservation,
                                  restaurant: restaurant,
                                  business_period: business_period,
                                  reservation_datetime: 1.day.from_now.change(hour: 15),
                                  party_size: 2)
        
        service = ReservationAllocatorService.new(limited_reservation)
    ...
    context '邊界情況測試' do
      ...
      it '跨日期的同餐期不衝突' do
        # 今天的午餐
        today_reservation = create(:reservation, :confirmed,
                                 restaurant: restaurant,
                                 business_period: business_period,
                                 table: table1,
                                 reservation_datetime: Date.current.change(hour: 12),
                                 party_size: 2)
        
        # 明天的午餐
        tomorrow_reservation = build(:reservation,
                                   restaurant: restaurant,
                                   business_period: business_period,
                                   reservation_datetime: 1.day.from_now.change(hour: 12),
                                   party_size: 2)
        
        service = ReservationAllocatorService.new(tomorrow_reservation)
