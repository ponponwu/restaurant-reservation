...
RSpec.describe ApplicationHelper, type: :helper do
  ...
  describe '側邊導航 helper 方法' do
    ...
    describe '#is_reservation_periods_page?' do
      it '當在營業時段頁面時回傳 true' do
        allow(helper.request).to receive(:path).and_return("/admin/restaurants/#{restaurant.id}/reservation_periods")
        expect(helper.is_reservation_periods_page?(restaurant)).to be true
      end

      it '當不在營業時段頁面時回傳 false' do
        allow(helper.request).to receive(:path).and_return("/admin/restaurants/#{restaurant.id}")
        expect(helper.is_reservation_periods_page?(restaurant)).to be false
      end
    end
