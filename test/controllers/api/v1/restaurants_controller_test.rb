  ...
  module V1
    class RestaurantsControllerTest < ActionDispatch::IntegrationTest
      ...
      setup do
        ...
        @reservation_period = reservation_periods(:lunch)
        @table = tables(:table_one)

        # 確保餐廳有關聯的營業時段和桌位
        @restaurant.reservation_periods << @reservation_period
      ...
      test 'should get available dates' do
        ...
        travel_to Time.zone.local(2025, 6, 10, 12, 0, 0) do
          ...
          @reservation_period.update!(days_of_week: { monday: true, tuesday: true, wednesday: true,
          ...
          assert_not_nil json_response['reservation_periods']
        end
      end
      ...
      test 'should handle weekly closure days' do
        ...
        travel_to Time.zone.local(2025, 6, 10, 12, 0, 0) do
          # 設定每週一公休
          @reservation_period.update!(days_of_week: { monday: false, tuesday: true, wednesday: true,
                                                   thursday: true, friday: true, saturday: true, sunday: true })
