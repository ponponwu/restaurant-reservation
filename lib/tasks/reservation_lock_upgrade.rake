namespace :reservation_lock do
  ...
  task verify_upgrade: :environment do
    ...
    begin
      ...
      if ReservationLockService.upgraded?
        ...
        if redis_healthy?
          puts "✅ Redis 連接正常"
          puts "   Redis 版本: #{Redis.current.info['redis_version']}" rescue "無法獲取版本"
        else
        ...
        # 檢查新功能
        puts "\n🆕 新功能檢查："
        methods_to_check = [:locked?, :force_unlock, :active_locks]
  ...
  task test_lock_service: :environment do
    ...
    # 在非測試環境中檢查 Redis 可用性
    unless Rails.env.test? || redis_healthy?
      puts "⚠️  Redis 不可用，將使用測試模式運行"
    end
